...
# 不上传给 DeepSeek 的字段（为了节省 Token，除了原有的，也可以把解析过滤掉）
STRIP_FIELDS = {
    "prototype",
    "fuzzywuzzy_doubt",
...
# ----------------------------
# Utility: Response JSON extraction (2D Array)
# ----------------------------
...
def needs_test_point(q: Dict[str, Any]) -> bool:
    ...
    # 如果不存在，或者为空字符串，或者为空列表，或者为 None，则需要补
...
# ----------------------------
# Ordered insertion: put test_point right after type
# ----------------------------
...
def normalize_response_items(parsed: List[List[Any]]) -> Dict[str, List[int]]:
    """
    DeepSeek 返回：[[1052, 31, 33], [1053, 25], ...]
...
def refresh_one_type_test_point(
    ...
    log_line(
        f"\n==> 开始生成考点 {target_type}: batch_size={batch_size}, "
        f"总题数={len(bank)}, 待补考点={len(pending)}, 批次数={len(batches)}, "
    ...
    for bi, batch_questions in enumerate(batches, start=1):
        ...
        try:
            # 考点提取属于结构化分类任务，temperature 设低一些 (0.1)
            ...
            log_line(f"[OK] {target_type} 第 {bi} 轮：更新 {batch_updated} 题考点，累计更新 {updated_count}")
            time.sleep(sleep_s)
    ...
    log_line(f"==> 完成 {target_type}: 本次共更新考点 {updated_count} 题 -> {in_path}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate test_point fields for new_bank files via DeepSeek.")
    p.add_argument(
        "--banks",
        type=str,
        default="",
...
def select_banks_interactive() -> List[str]:
    log_line("请选择要生成考点的题库（如：A4 B X 或 A4,B,X 或 all）：")
    s = input("> ").strip()
    if not s:
        return []
    if s.lower() == "all":
...
def main() -> None:
    ...
    log_line(f"[OK] 本次选择生成考点：{targets}")
    ...
    for t in BANK_ORDER:
        ...
        try:
            ...
            log_line(f"[ERROR] 生成考点 {t} 发生未捕获异常，已跳过该题库，继续下一个。")
