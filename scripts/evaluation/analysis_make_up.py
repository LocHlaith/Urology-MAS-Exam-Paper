...
MAIN_LOG_PATH = os.path.join(
    LOG_DIR,
    f"analysis_make_up_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
...
def make_batch_log_path(target_type: str, batch_index: int) -> str:
    ...
    return os.path.join(LOG_DIR, f"analysis_make_up_{target_type}_batch_{batch_index:04d}_{ts}.log")
...
# ----------------------------
# Per-type analysis/answer keys
# ----------------------------

def analysis_keys_for_type(t: str) -> List[str]:
    # 与 analysis.py 一致的“期望字段”
    if t in ("A1", "A2", "X"):
        return ["analysis"]
    if t == "A3":
...
def needs_make_up(q: Dict[str, Any], t: str) -> bool:
    """
    补救规则：
    - 一般：只要任意 analysis* 缺失或为空 -> 需要补救
    - 特例(B)：若同时有 analysis1 与 analysis2（非空），即使没有 analysis3，也视为正常 -> 不补救
    """
    if t == "B":
        a1_ok = _nonempty_str(q.get("analysis1", ""))
        a2_ok = _nonempty_str(q.get("analysis2", ""))
        if a1_ok and a2_ok:
            return False  # 特例：允许缺 analysis3
        return True
...
# ----------------------------
# Ordered insertion: analysis right after answer
# ----------------------------
...
def insert_analysis_after_answer(q: Dict[str, Any], t: str, patch: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k in an_keys:
        v = patch.get(k, "")
        patch_clean[k] = v.strip() if isinstance(v, str) else ""
...
def normalize_response_items(t: str, parsed: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
...
def refresh_one_type_make_up(
    ...
    bank = read_json_list(in_path)

    # 只挑“需要补救”的题
    pending: List[Dict[str, Any]] = [q for q in bank if needs_make_up(q, target_type)]
    batches = build_batches(pending, batch_size=batch_size)

    log_line(
        f"\n==> 开始补救解析 {target_type}: batch_size={batch_size}, "
        f"总题数={len(bank)}, 待补救={len(pending)}, 批次数={len(batches)}, "
    ...
    for bi, batch_questions in enumerate(batches, start=1):
        ...
        log_line(f"  - {target_type} 第 {bi}/{len(batches)} 轮：补救题数={len(batch_questions)}")
        ...
        try:
            ...
            log_line(f"[OK] {target_type} 第 {bi} 轮：补救更新 {batch_updated} 题，累计补救 {updated_count}")
            time.sleep(sleep_s)
    ...
    log_line(f"==> 完成补救 {target_type}: 本次共补救更新 {updated_count} 题 -> {in_path}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Make-up program: regenerate missing analysis fields via DeepSeek.")
    p.add_argument(
        "--banks",
        type=str,
        default="",
...
def select_banks_interactive() -> List[str]:
    log_line("请选择要补救解析的题库（如：A4 B X 或 A4,B,X 或 all）：")
    s = input("> ").strip()
    if not s:
        return []
    if s.lower() == "all":
...
def main() -> None:
    ...
    for t in BANK_ORDER:
        ...
        try:
            ...
            log_line(f"[ERROR] 补救 {t} 发生未捕获异常，已跳过该题库，继续下一个。")
            log_block(MAIN_LOG_PATH, f"UNCAUGHT EXCEPTION in refresh_one_type_make_up({t})", traceback.format_exc())
            continue
