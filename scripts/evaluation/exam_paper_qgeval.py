"""为用户明确提供的试卷 JSON 写入 QGEval 机器评分。

脚本用途：调用模型按 QGEval 维度为组卷后试卷题目打分。
流程阶段：试卷机器评价。
主要输入：用户通过 `--target-file` 明确指定的试卷 JSON，以及 `prompts/evaluation/prompt_for_qgeval.txt`。
主要输出：原地更新的试卷 JSON，写入 `QGEval` 字段。
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="为指定试卷 JSON 写入 QGEval 机器评分。")
    p.add_argument(
        "--target-file",
        "--target_file",
        dest="target_file",
