"""为人类题库或 MAS 题库写入 ULM rubric 机器评分。

脚本用途：调用模型按医学教育/题目质量 rubric 为题库题目打分。
流程阶段：题库机器评价。
主要输入：`data/banks/bank_*.json` 或 `data/banks/new_bank_*.json`，以及 `prompts/evaluation/prompt_for_ulm.txt`。
主要输出：原地更新的题库 JSON，写入 `ULM` 字段。
...
MAIN_LOG_PATH = os.path.join(
    LOG_DIR,
    f"ulm_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
...
def make_batch_log_path(bank_set: str, bank_type: str, batch_index: int) -> str:
    ...
    return os.path.join(LOG_DIR, f"ulm_{bank_set}_{bank_type}_batch_{batch_index:04d}_{ts}.log")
...
def eval_one_bank(
    ...
    for bi, batch_questions in enumerate(batches, start=1):
        ...
        try:
            ...
            log_line(f"[OK] {bank_set}:{bank_type} 第 {bi} 批：写入 ULM={updated}，返回行={len(rows)}，未命中id={missed}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="为人类题库或 MAS 题库写入 ULM rubric 机器评分。")
    p.add_argument(
        "--bank_set",
        type=str,
        default="",
