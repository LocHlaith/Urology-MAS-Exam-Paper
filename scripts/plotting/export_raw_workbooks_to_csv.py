#!/usr/bin/env python3
"""Export every plotting raw-data XLSX worksheet to CSV, then optionally delete XLSX.
...
from project_paths import PLOT_DERIVED_DATA_DIR, PLOT_RAW_DATA_DIR  # noqa: E402

RAW_DATA = PLOT_RAW_DATA_DIR
...
def main() -> None:
    ...
    parser.add_argument(
        "--delete-originals",
        action="store_true",
        help="Delete the explicitly enumerated plot/data/raw XLSX files after verification.",
    )
    args = parser.parse_args()

    sources = sorted(
        path
        for path in RAW_DATA.rglob("*.xlsx")
        if not path.name.startswith("~$")
    )
    if not sources:
        print("No plot/data/raw XLSX files found; existing CSV exports were left unchanged.")
        return
    ...
    if args.delete_originals:
        for source in sources:
            ...
            if RAW_DATA.resolve() not in resolved.parents or resolved.suffix.lower() != ".xlsx":
