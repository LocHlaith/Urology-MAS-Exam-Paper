...
# 只处理 temp/B.json
BANK_FILE = os.path.join(ROOT, "B.json")

PROMPT_LLM_PATH = os.path.join(os.path.dirname(ROOT), "prompt_for_llm.txt")
...
def eval_temp_bank(
    ...
    log_line(f"==> temp/B.json: 总题数={len(data)}, 待评分={len(pending)}, batch_size={BATCH_SIZE}, start_batch={start_batch}")

    if not pending:
        log_line(f"[OK] temp/B.json: 无待评分题目，跳过。")
        return
    ...
    for idx, q in enumerate(data):
        ...
        try:
            ...
            raise ValueError(f"temp/B.json 出现无法解析为 int 的 id：{qid_raw!r}")
        id_to_pos[qid] = idx

    batches = split_batches(pending, BATCH_SIZE)

    bak = backup_file(path)
    log_line(f"[OK] temp/B.json: 已备份原文件 -> {bak}")
    ...
    for bi, batch_questions in enumerate(batches, start=1):
        ...
        log_block(
            batch_log_path,
            title=f"BATCH START | bank=temp/B.json | batch={bi}/{len(batches)} | n={len(batch_questions)}",
        ...
        log_line(f"  - temp/B.json 第 {bi}/{len(batches)} 批：n={len(batch_questions)}")
        ...
        try:
            ...
            log_line(f"[OK] temp/B.json 第 {bi} 批：写入 LLM={updated}，返回行={len(rows)}，未命中id={missed}")

            time.sleep(sleep_seconds)

        except Exception as e:
            err_text = f"{type(e).__name__}: {e}"
            log_line(f"[SKIP] temp/B.json 第 {bi} 批失败，已跳过：{err_text}")
    ...
    log_line(f"==> temp/B.json: 完成。已评分={done}/{len(final_data)}，输出文件={path}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Add LLM scores into temp/B.json using DeepSeek.")
    p.add_argument(
        "--start_batch",
        type=int,
        default=1,
...
def main() -> None:
    ...
    log_line(f"[OK] 本次选择评分：bank=temp/B.json | start_batch={args.start_batch} | batch_size={BATCH_SIZE}")
    ...
    try:
        ...
        log_line(f"[ERROR] temp/B.json 评分发生未捕获异常，程序将正常结束。")
