...
# 目标文件
TARGET_BANK_FILE = os.path.join(ROOT, "B.json")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Add QGEval scores into temp/B.json using DeepSeek.")
    p.add_argument(
        "--start_batch",
        type=int,
        default=1,
...
def main() -> None:
    ...
    # Load system prompt
    prompt_path = os.path.join(os.path.dirname(ROOT), "prompt_for_qgeval.txt")
    try:
