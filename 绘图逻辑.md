...
专家综合质量评估的详细流程图，包括QGEval评分、ULM评分、图灵测试、Major Defect等。
...
比较专家在QGEval 7维度和ULM 16维度每个维度上的（平均分±标准差）之间的差异显著性。δ = -0.30（5分制），δ = -0.25（4分制），δ = -0.20（3分制），分制详见prompts\evaluation\prompt_for_qgeval.txt、prompts\evaluation\prompt_for_ulm.txt。模仿上图的(B)，共23维度。上面7个QGEval维度和下面16个ULM维度之间稍微分开一些；UroEMAS未显著低于Human则标注n.s.，上图的(B)中有的其他元素也应在我们图中出现。

### 2D

展现不同认知层级的题目的专家质量评分差，差异显著性分析，模仿下图（另一篇论文的图片），但我们有QGEval和ULM两种评分方式，因此要用两种颜色。
...
UroEMAS与Human卷的克伦巴赫指数图，画法是dot plot with bootstrap CI，分别是整张试卷、UroEMAS卷和Human卷，画法类似下图：
...
专家guessed_UroEMAS模型森林图。
（以下内容供参考）：
"guessed_UroEMAS 模型"指什么
guessed = "被猜测为AI/被判定为人写"——即被试对每道题给出"我认为这是 AI 出的"判断（0/1 或 vote proportion）
UroEMAS（多智能体系统）可用多个ULM Agent模拟人类评判者，对题目做"人写 vs AI写"投票，汇总得到guessed概率/标签。
guessed_UroEMAS 模型 = 以guessed（UroEMAS投票结果或人类投票汇总）为因变量（DV），建立二分类模型：
guessed_UroEMAS ~ actual_source + question_type + difficulty + discipline + (1 | expert) + (1 | item)
即检验：题目真实来源（人/AI）、认知层级及其交互是否影响题目被判断为AI的概率。
